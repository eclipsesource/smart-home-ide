...
export class JUnitRunService {
    ...
    private findSourceDirectory  (javaUri: URI): URI | undefined  {
        ...
        while (!sourceDir.path.isRoot && sourceDir.path.name !== 'src' && sourceDir.path.name !== 'src-gen' && sourceDir.path.name !== 'test') {
            sourceDir = sourceDir.parent
        }
        if (sourceDir.path.name === 'src' || sourceDir.path.name === 'src-gen'|| sourceDir.path.name === 'test') {
            return sourceDir
        }
        return undefined;
    }
